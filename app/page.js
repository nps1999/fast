...
const CURRENCIES = {
  USD: { symbol: '$', name: 'دولار أمريكي', flagCode: 'us' }, SAR: { symbol: 'ر.س', name: 'ريال سعودي', flagCode: 'sa' },
  KWD: { symbol: 'د.ك', name: 'دينار كويتي', flagCode: 'kw' }, AED: { symbol: 'د.إ', name: 'درهم إماراتي', flagCode: 'ae' },
  BHD: { symbol: 'د.ب', name: 'دينار بحريني', flagCode: 'bh' }, QAR: { symbol: 'ر.ق', name: 'ريال قطري', flagCode: 'qa' },
  OMR: { symbol: 'ر.ع', name: 'ريال عماني', flagCode: 'om' },
};
      ...
      <div className="container mx-auto px-4 py-3"><div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={currency} onValueChange={changeCurrency}>
            ...
            <SelectContent className="bg-[#12121f] border-purple-500/20">{Object.entries(CURRENCIES).map(([k,v]) => <SelectItem key={k} value={k}><span className="flex items-center gap-2"><img src={`https://flagcdn.com/16x12/${v.flagCode}.png`} alt={k} className="w-4 h-3 object-cover rounded-sm" /> {v.symbol} {k} - {v.name}</span></SelectItem>)}</SelectContent>
          </Select>
